 ...
 */
public class SalesGoodsImg extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // 表示する商品コードの取得
        String[] goodsCd = request.getParameterValues("goodsCd");

        GoodsMstImgCache mst;

        try {

            mst = GoodsMstImgCache.getInstance();

            if (mst != null) {
                if (mst.get(goodsCd[0]) != null) {
                    if (mst.get(goodsCd[0]).getImgPhoto() != null) {
                        /* 画像ファイルを 読取 */
                        inDataByte = mst.get(goodsCd[0]).getImgPhoto();
                    }
                }
            }
